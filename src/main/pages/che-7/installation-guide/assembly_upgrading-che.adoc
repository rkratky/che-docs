...
WARNING: The method that was used to install the {prod-short} instance determines the method to proceed with for the upgrade.

include::assembly_upgrading-{prod-id-short}-on-openshift-4-using-the-operator.adoc[leveloffset=+1]

include::proc_upgrading-{prod-id-short}-on-openshift-4-using-the-cli-management-tool.adoc[leveloffset=+1]

include::proc_upgrading-{prod-id-short}-on-openshift-3-using-the-cli-management-tool.adoc[leveloffset=+1]
...
[subs="+quotes,+attributes"]
----
$ oc patch checluster _<my-{prod-id-short}>_ --type=json -n _<{prod-id-short}-namespace>_ -p '...'
----

WARNING: Making local updates to the YAML file of the `checluster` resource and then applying such changed resource to the cluster using `kubectl apply -f` or `oc apply -f` can result in an invalidation of the {prod-short} installation.
